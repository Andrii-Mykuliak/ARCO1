...
def axis1_algorithm(cfg, X, U, labels, texts=None) -> pd.DataFrame:
    ...
    def _compute():
        ...
        if absent:
            ...
            log(f"!! AXIS 1 INCOMPLETE - {len(absent)} of {expected} judges did not run: "
                f"{', '.join(absent)}")
            log("!! Axis-1 scores are a fraction of the judges that ran, so these "
        ...
        log(f"axis1: {len(alts)} of {expected} paper judges ran"
...
# --------------------------------------------------------------- axis 2 ----
def axis2_bootstrap(cfg, X, labels) -> pd.DataFrame:
...
def held_out_centroid_assignment_diagnostic(cfg, X, labels,
    ...
    def _compute():
        ...
        log(f"axis3: {len(train_s)} train / {len(uniq) - len(train_s)} test sessions")
...
# --------------------------------------------------------------- axis 4 ----
def axis4_hierarchical(cfg, X, labels) -> pd.DataFrame:
    ...
    def _compute():
        ...
        for b in range(cfg.axis4_n_replicates):
            ...
            log(f"  axis4 replicate {b + 1}/{cfg.axis4_n_replicates} "
                f"({len(sub)} sentences)")
        rows = [{"cluster": c,
...
def combine_axes(cfg, a1, a2, a3, a4=None) -> pd.DataFrame:
...
def axis_counts(v: pd.DataFrame) -> pd.DataFrame:
...
def axis_correlations(v: pd.DataFrame) -> pd.DataFrame:
