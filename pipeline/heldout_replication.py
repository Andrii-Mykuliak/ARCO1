...
def _stage_a(cfg, embeddings: np.ndarray, sessions: pd.Series,
             canonical: np.ndarray, split=None) -> dict:
    """Train-only refit, alignment, and held-out projection.

    ``split`` supplies the train/held-out race ids explicitly, for callers that
    sweep the split itself. Left as None, the reference split of ``race_split``
    is used and the behaviour is exactly as before.
    """
    import hdbscan
    import umap
    ...
    if split is None:
        ...
        train_races, test_races = (list(split[0]), list(split[1]))
    ...
    log(f"held-out replication stage A: {len(train_races)} train / {len(test_races)} held-out sessions (mcs={mcs}); "
...
def axis3_heldout_replication(cfg, df: pd.DataFrame, embeddings: np.ndarray,
                              ...
                              text_col: str = "masked",
                              split=None, return_stage=False):
    """Per-cluster held-out race replication over one race-level split.

    ``split`` and ``return_stage`` serve the factorial sweep: the first supplies
    the split explicitly, the second returns the stage-A masks and labels so the
    caller can derive its own run-level quantities. Neither changes the
    reference-split result, and neither is cached.
    """
    stage = {}
    ...
    def _compute():
        ...
        A = _stage_a(cfg, embeddings, sessions, canonical, split=split)
        stage.update(A)
        ...
        log(f"held-out replication: {len(ids)} clusters have both training and held-out support")
        ...
        log(f"held-out replication: {int(out.axis3_pass.sum())}/{len(out)} clusters meet "
            f"rep_strict >= 0.5 AND rep_dir >= 0.5")
        return out

    # The reference split is the canonical, cached result. A supplied split is a
    # sweep member: same computation, never cached under the canonical key.
    if split is None and not return_stage:
        return cached(cfg, "axis3_heldout.csv", _compute)
    out = _compute()
    return (out, stage) if return_stage else out
